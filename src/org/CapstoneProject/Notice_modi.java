...
public class Notice_modi extends Dialog implements ActionListener, MouseListener {
...
//	JMenu Mn_Logout = new JMenu(" 로그아웃 ");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == bCancel) {
			...
			int result = JOptionPane.showConfirmDialog(null, "공지사항을 수정하시겠습니까?", "공지사항 수정 확인", JOptionPane.YES_NO_OPTION, JOptionPane.INFORMATION_MESSAGE);
			if(result == 0) {
				NoticeData.updateN(tTitle.getText(), tContent.getText(), POST_NUM);
				JOptionPane.showMessageDialog(null, "공지사항이 수정되었습니다.", "공지사항 수정", JOptionPane.INFORMATION_MESSAGE);
				dispose();
			}
			else {
