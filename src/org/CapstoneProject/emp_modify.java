...
public class emp_modify extends JPanel implements ActionListener {
	...
	String EMP_NO, EMP_NM, EMP_DT, EMP_PH_NUM, EMP_TP, EMP_ADDR, REG_TP, REG_ID, REG_PW, sid, sid2, EMP_NUM, 
			admin_num, admin_nm, admin_appc_dt, admin_id;
...
//	     super(fr, "", true);
		...
		admin_num = Login.admin_num;
		admin_nm = Login.admin_nm;
		admin_appc_dt = Login.admin_appc_dt;
...
//		mb_regist2.setPreferredSize(new Dimension(200, 28));
		...
		Temp_no.setText(admin_num);
		Temp_no.setEnabled(false);
		Temp_no.setPreferredSize(new Dimension(100, 30));
		Temp_nm = new JTextField(22);
		Temp_nm.setText(admin_nm);
		Temp_nm.setEnabled(false);
		Temp_nm.setPreferredSize(new Dimension(100, 30));
		Temp_dt = new JTextField(22);
		Temp_dt.setText(admin_appc_dt);
		Temp_dt.setEnabled(false);
		...
		Treg_id.setText(admin_id);
		Treg_id.setEnabled(false);
...
//				CBreg_tp = new JComboBox<String>(empty);		
			...
			int result = JOptionPane.showConfirmDialog(null, "해당 사원 정보를 수정하시겠습니까?", "사원 등록 확인",
			...
			if (result == 0) {
				...
				if (EMP_NUM.equals("") || EMP_NM.equals("") || EMP_DT.equals("") || EMP_ADDR.equals("") || EMP_PH_NUM.equals("")
					...
					empData.createemp();
					JOptionPane.showMessageDialog(null, "사원정보가 수정되었습니다.", "사원 등록", JOptionPane.WARNING_MESSAGE);
//					dispose();
							Temp_no.setText("");
							Temp_nm.setText("");
							Temp_dt.setText("");
