...
public class purData {
	...
	static String quary, quary2, quary_SEQ, quary_commit;
	...
	static void createpur() {
		...
		quary = "INSERT INTO PUR VALUES(SEQ_PUR_NUM.NEXTVAL,'" + purData.get("거래처번호") + "', '" + purData.get("구매일자")
				+ "' ," + purData.get("총금액") + ")";
		purListData.clear();
		quary_commit = "commit";
	...
	static List<Map<String, Serializable>> searchSEQ() {
		...
		try {
			pstm = conn.prepareStatement(quary2, rs.TYPE_SCROLL_INSENSITIVE, rs.CONCUR_READ_ONLY);
			rs = pstm.executeQuery();
		} catch (SQLException sqle) {
			System.out.println("select문에서 예외 발생");
			sqle.printStackTrace();
