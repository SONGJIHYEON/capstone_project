...
public class ProImageType extends JPanel implements ActionListener, MouseListener {
	...
	private static JLabel label1, labelv1, labelv2;
...
//      setLayout(new FlowLayout(FlowLayout.CENTER, 100, 50));      
	...
	void getData() {
		...
		labelv1 = new JLabel("");
		labelv1.setPreferredSize(new Dimension(200, 200));
		labelv2 = new JLabel("");
		labelv2.setPreferredSize(new Dimension(200, 200));
		...
		for (int i = nowPage * postPerPage; i < nowPage * postPerPage + postPerPage; i++) {
			if (i > ImageListData.size() - 1) {
				if (ImageListData.size() % 3 == 1) {
					Pimg.add(labelv1);
					Pimg.add(labelv2);
					break;
				} else if (ImageListData.size() % 3 == 2) {
					Pimg.add(labelv1);
					break;
				} else {
					break;
	...
	public void mouseClicked(MouseEvent e) {
		...
		for (int j = 0; j < arModelImg.size(); j++) {
			if (e.getSource() == ImgLabels[j]) {
				String fileName = arModelImg.get(j).toString();
