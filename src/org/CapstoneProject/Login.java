...
public class Login extends JPanel implements ActionListener, MouseListener {
	...
	private JTextField xEmpName, xEmpPhone, xhEmpRegist, xEmpHireDate, xDeptName, xEmpSal, xSpvName, xEmpAnin, xEmpComm,
			xId, xPwd;

	private String[] col1 = { "부서명", "부서위치" };
	private String[] col2 = { "부서명", "성명" };
	private String[] div = { "회원", "관리자" }; // 사원구분 콤보박스의 목록
	...
	static String admin_num, admin_nm, admin_appc_dt, admin_id, user_num, user_nm, user_id, user_grade, user_disc_rt,
			user_point, user_addr, user_phone, user_birth;
	static int Login = 0;
	...
	private void getData(List<Map<String, Serializable>> idListData) {
		...
		for (int i = 0; i < idListData.size(); i++) {
			...
			id.put(idListData.get(i).get("ID"), idListData.get(i).get("PWD"));
		}
	...
	public Login() {
		...
		cbSel.setFont(new Font("휴먼매직체", Font.BOLD, 18));
		cbSel.setBackground(Color.WHITE);
		cbSel.setPreferredSize(new Dimension(153, 30));

		vId = new JLabel("아이디");
		vId.setFont(new Font("휴먼매직체", Font.PLAIN, 20));
		vId.setPreferredSize(new Dimension(80, 30));

		vPwd = new JLabel("비밀번호");
		vPwd.setFont(new Font("휴먼매직체", Font.PLAIN, 20));
		vPwd.setPreferredSize(new Dimension(80, 30));

		xId = new JTextField(15);
		xId.setPreferredSize(new Dimension(153, 30));

		xPwd = new JPasswordField(15);
		xPwd.setPreferredSize(new Dimension(153, 30));
...
//      gridbagconstraints.weighty=1.0;
		gridbagAdd(vId, 0, 3, 2, 1);
		gridbagAdd(vPwd, 0, 4, 2, 1);

		gridbagconstraints.anchor = GridBagConstraints.CENTER;

		gridbagAdd(vLabel1, 1, 0, 2, 1);
		gridbagAdd(vLabel2, 1, 1, 2, 1);
		gridbagAdd(cbSel, 1, 2, 2, 1);
		gridbagAdd(xId, 1, 3, 2, 1);
		gridbagAdd(xPwd, 1, 4, 2, 1);
		gridbagAdd(BtLogin, 3, 3, 2, 2);
		gridbagAdd(vLabel3, 1, 5, 2, 1);
		gridbagAdd(vLabel4, 1, 6, 2, 1);
		gridbagAdd(vLabel5, 1, 7, 2, 1);
